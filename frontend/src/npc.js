class Npc{
   ...
   handleListClick = (e) => {
       ...
       if (e.target.className === "delete"){
           ...
           this.addUpdateNpcFields(id)
       }
   }

   updateNpcOnDom({name, race, sex, alignment, melee, ranged, health, armor, strength, dexterity, constitution, wisdom, charisma, encounter_id}){
    this.name = name
    this.race = race
    this.sex = sex
    this.alignment = alignment
...
addUpdateNpcFields(npcId){
    let npc = document.querySelector(`#npc-full-info-${npcId}`)
    let updateForm = `
    <fieldset id="npc-full-info-${this.id}">
        <legend style="text-transform:uppercase" class="name" ><b>${this.name}</b></legend>
        The <span class="alignment">${this.alignment}</span>
